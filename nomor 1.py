...
counter = 0.0  # Inisialisasi counter waktu simulasi

for _ in range(t_n):  # Loop sebanyak iterasi simulasi
    for i in range(1, node-1):  # Melooping setiap titik grid kecuali batas
        u[i] = (dt * a * (w[i-1] - 2*w[i] + w[i+1]) / dx**2) + w[i]  # Perhitungan suhu baru
    ...
    plt.pause(0.1)  # Memberi sedikit waktu untuk memperbarui plot
